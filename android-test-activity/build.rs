...
/// Get the list of enabled features from the current build
fn get_enabled_features() -> String {
    ...
    for (key, _value) in env::vars() {
        if let Some(feature) = key.strip_prefix("CARGO_FEATURE_") {
            // Convert CARGO_FEATURE_SDK_OS_BUILD to sdk_os_build
            let feature_name = feature.to_lowercase();
            println!("Enabled feature: {}", feature_name);
            features.push(feature_name);
        } else {
            println!("Non-feature env var: {}={}", key, _value);
        }
    }
...
fn main() {
    ...
    let java_src =
        PathBuf::from("app/src/main/java/io/github/jni_rs/jbindgen/testactivity/TestActivity.java");
    ...
    */
    ...
    let bindings = jbindgen::Builder::new()
        .input_sources(
            ...
            vec!["io.github.jni_rs.jbindgen.testactivity.TestActivity".to_string()],
        )
        .generate()
    ...
    println!(
        "cargo:rustc-env=ANDROID_TEST_ACTIVITY_FEATURES={}",
        get_enabled_features()
    );
}

fn download_hiddenapi_flags() -> PathBuf {
    let tmp_dir = env::var("OUT_DIR").expect("OUT_DIR is set");
    ...
    if !hiddenapi_flags_path.exists() {
        ...
        std::io::copy(&mut reader, &mut file).expect("Failed to write hiddenapi-flags.csv");

        println!(
            "cargo:warning=Downloaded hiddenapi-flags.csv to {}",
            hiddenapi_flags_path.display()
        );
    } else {
        println!(
            "cargo:warning=Using cached hiddenapi-flags.csv from {}",
            hiddenapi_flags_path.display()
        );
    }
...
fn generate_sdk_binding_with_skip(
    ...
    let mut builder = Builder::new()
        ...
        .hiddenapi_flags(
            hiddenapi_flags_path
                .to_str()
                .expect("Invalid hiddenapi_flags_path"),
        );
