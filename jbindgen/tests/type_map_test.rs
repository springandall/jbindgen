...
fn test_bindings_type_map() {
    ...
    let bindings = Builder::new()
        ...
        .type_mapping(
            "external::crate::SomeType".to_string(),
            "com.external.SomeType".to_string(),
        )
        .generate()
        .unwrap();
    ...
    assert!(
        type_map
            .get_rust_type("com.example.WithDependencies")
            .is_some()
    );
    ...
    let simple_class_type = pub_type_map
        .get_rust_type("com.example.SimpleClass")
        .unwrap();
    assert!(
        simple_class_type.starts_with("my_crate::"),
        "Expected public root 'my_crate::' but got: {}",
        simple_class_type
    );
    ...
    bindings
        .write_pub_type_map(&pub_type_map_file, "my_crate")
        .unwrap();
